...
@onready var texture_button: TextureButton = $MarginContainer2/Control/TextureButton
...
func _on_button_pressed() -> void:
	if file_path:
		...
		if file == null:
			push_error("Failed to open the .exe file.")
			return
		
		var buffer = file.get_buffer(file.get_length())
		file.close()
		
		var file_name = file_path.replace("games/", "")
		JavaScriptBridge.download_buffer(buffer, file_name, "application/octet-stream")
	
	if link:
		var js_code = "window.open('%s', '_blank').focus();" % [link]
